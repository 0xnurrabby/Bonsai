...
function canWater(st: LocalState) {
  return hoursSince(st.lastWateredAt) >= 0.1;
}
...
export default function App() {
  ...
  async function sendGameTx(intent: "plant" | "water" | "revive" | "graft", dataHex?: string) {
    ...
    try {
      ...
      // Gas sanity-check (misleading wallet errors happen when balance is 0)
      ...
      const data = encodeLogAction(intent, dataHex || "0x");

      // Pre-tx UX: 1–1.5s BEFORE wallet opens
      toast.show(intent === "water" ? "A breeze… then water." : "Ink settles… then onchain.");
      await sleep(1100);

      // 1) Prefer ERC-5792 (Base Build requirement) with builder attribution
      try {
        const payload = {
          version: "2.0.0",
          from,
          ...
          atomicRequired: true,
          calls: [{ to: GAME_CONTRACT, value: "0x0", data }],
        ...
        else toast.show("Submitted.");
        return true;
      } catch (e: any) {
        // 2) Fallback: direct tx (more likely to show under contract Transactions)
        const txHash = await provider.request({
          method: "eth_sendTransaction",
          params: [{ from, to: GAME_CONTRACT, value: "0x0", data }],
        });
