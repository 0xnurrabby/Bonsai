...
// ERC-5792: poll status so we can surface the transaction hash (helps users verify on BaseScan)
async function waitForCalls(provider: any, callsId: any, timeoutMs: number = 60_000) {
  ...
  while (Date.now() - started < timeoutMs) {
    try {
      ...
      // Many wallets return: { status: "PENDING"|"CONFIRMED"|"FAILED", receipts?: [{ transactionHash }] }
      const s = (status?.status || "").toString().toUpperCase();
      const receipts = status?.receipts || status?.receipt || status?.transactions;
      const txHash = Array.isArray(receipts)
        ? receipts.find((r: any) => r?.transactionHash)?.transactionHash
        : receipts?.transactionHash;

      if (s === "CONFIRMED" || s === "SUCCESS") {
        return { ok: true, txHash: typeof txHash === "string" ? txHash : undefined };
      }
      if (s === "FAILED" || s === "REVERTED") {
        return { ok: false, txHash: typeof txHash === "string" ? txHash : undefined };
      }
    } catch {
      // Some wallets may not support status polling; ignore and fall back.
...
export default function App() {
  ...
  async function ensureBase(): Promise<boolean> {
    ...
    try {
      ...
      // Strict: the app must execute game actions on Base Mainnet only.
      // If on any other chain (including Base Sepolia), attempt to switch.
      if (cid !== BASE_MAINNET) {
  ...
  async function sendGameTx(intent: "plant" | "water" | "revive" | "graft", dataHex?: string) {
    const provider = getProvider();
    if (!provider) {
      toast.show("No wallet provider found.");
      return false;
    ...
    try {
      ...
      if (balWei <= 0n) {
        toast.show("You need a small amount of ETH on Base for gas.");
        return false;
      }const txData = encodeLogAction(intent, dataHex || "0x");

// Pre-tx UX: 1–1.5s before wallet opens
toast.show(intent === "water" ? "A breeze… then water." : "Ink settles… then onchain.");
await sleep(1100);

// Prefer eth_sendTransaction (gives a direct txHash and shows clearly on scanners),
// fallback to ERC-5792 wallet_sendCalls if needed.
try {
  ...
  if (typeof txHash === "string" && txHash.startsWith("0x")) {
    toast.show(`Sent: ${txHash.slice(0, 10)}…`);
  } else {
    toast.show("Sent.");
  }
  return true;
} catch (e1: any) {
  const c = (e1?.code ?? e1?.data?.code);
  const m1 = (e1?.message || "").toLowerCase();

  // User rejected
  if (c === 4001 || m1.includes("user rejected") || m1.includes("rejected")) {
    toast.show("Canceled in wallet.");
    return false;
  }

  // If method not supported, try wallet_sendCalls
  const methodMissing =
    c === -32601 ||
    m1.includes("method not found") ||
    m1.includes("does not exist") ||
    m1.includes("not supported");

  if (!methodMissing) {
    // Any other error: fall through to sendCalls anyway, but keep message sane.
    // (Some wallets throw odd errors yet still support sendCalls.)
  }
  ...
  try {
    ...
    const callsId = await walletSendCalls(provider, payload);
    const st = await waitForCalls(provider, callsId);
    if (st?.txHash) toast.show(`Sent: ${st.txHash.slice(0, 10)}…`);
    else toast.show("Sent.");
    return true;
  } catch (e2: any) {
    const c2 = (e2?.code ?? e2?.data?.code);
    const m2 = (e2?.message || "").toLowerCase();
    if (c2 === 4001 || m2.includes("user rejected") || m2.includes("rejected")) {
      toast.show("Canceled in wallet.");
      return false;
    }
    throw e2;
  ...
  async function graft() {
    ...
    // Encode friend address as raw 20 bytes in the dynamic bytes field
    const dataHex = "0x" + addr.replace(/^0x/, "");
    const ok = await sendGameTx("graft", dataHex);
    if (!ok) return;
  ...
  async function sendTip() {
    ...
    try {
      ...
      const callsId = await walletSendCalls(provider, payload);
      const st = await waitForCalls(provider, callsId);
      if (st?.txHash) toast.show(`Tip sent: ${st.txHash.slice(0, 10)}…`);
